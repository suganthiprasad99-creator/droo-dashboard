...
import { CalendarDays, ChevronLeft, ChevronRight, Plus, Search, SlidersHorizontal, Trash2, X } from 'lucide-react'
...
const STORAGE_KEY = 'droo.scheduler.entries.v1'
...
export function SchedulerPage() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try { setSchedules(JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]') as Schedule[]) } catch { setSchedules([]) }
      setStorageReady(true)
    }, 0)
    return () => window.clearTimeout(timer)
  }, [])

  useEffect(() => { if (storageReady) localStorage.setItem(STORAGE_KEY, JSON.stringify(schedules)) }, [schedules, storageReady])
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setSchedules(current => [...current, ...seeded.filter(entry => !current.some(item => item.id === entry.id))])
    }, 0)
    return () => window.clearTimeout(timer)
  }, [drivers, storageReady, weekStart])
    ...
    <section className="scheduler-panel">
      ...
      {loading ? <EmptyState loading /> : <div className="scheduler-grid-wrap"><div className="scheduler-grid" style={{ gridTemplateColumns: '210px repeat(7, minmax(130px, 1fr))' }}>
        ...
        {filteredDrivers.map(driver => { const id = String(driver.id); return <div className="scheduler-row-contents" key={id}>
          ...
          {days.map(day => { const key = dateKey(day); const entry = schedules.find(item => item.driverId === id && item.date === key); return entry ? <button className={`schedule-cell ${entry.status}`} key={key} onClick={() => openEditor(id, key, entry)}><strong>{entry.status === 'off' ? 'OFF' : `${entry.start} – ${entry.end}`}</strong>{entry.route && <span>{entry.route}</span>}</button> : <button className="schedule-cell empty" key={key} onClick={() => openEditor(id, key)}><Plus /><span>Add</span></button> })}
        </div> })}
