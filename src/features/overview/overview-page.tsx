...
export function OverviewPage() {
  ...
  return <div className={`shot-dashboard ${editingLayout ? 'dashboard-layout-editing' : ''}`}>
    ...
    <section className="shot-orders-load">
      ...
      <article className="shot-vehicle-load"><header><h2>Vehicle Load</h2><span>• {currentVehicleDriver?.online?'Live':'Offline'}</span></header><div className="truck-stage"><button aria-label="Previous vehicle" onClick={()=>moveVehicle(-1)}><ChevronLeft/></button><div className="vehicle-load-truck" style={{'--vehicle-load':`${currentLoadPercent}%`} as React.CSSProperties}><Image src="/images/vehicle-load-truck-empty.png" alt="White cargo truck showing the current vehicle load" width={1868} height={842} priority/><div className="vehicle-load-fill" aria-hidden="true"/><strong>{currentLoadPercent}%</strong></div><button aria-label="Next vehicle" onClick={()=>moveVehicle(1)}><ChevronRight/></button></div><strong className="capacity">{currentLoadPercent}%</strong><span>capacity</span><small>VEHICLE</small><b>{String(currentVehicle.registration_number||'No vehicle')}</b><small>DRIVER</small><b>{String(currentVehicleDriver?.name||'—')}</b><div className="load-numbers"><p><span>CURRENT LOAD</span><strong>{currentLoadKG} KG</strong></p><p><span>MAX LOAD</span><strong>{maxLoadKG} KG</strong></p></div><footer>Vehicle {vehicles.length?vehicleIndex+1:0} of {vehicles.length}<div>{vehicles.map((vehicle,index)=>index===vehicleIndex?<b key={String(vehicle.id)}/>:<i key={String(vehicle.id)} onClick={()=>setVehicleIndex(index)}/>)}</div></footer></article>
    </section>
