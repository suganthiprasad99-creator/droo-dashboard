...
const fromApi = (row: ApiWorkOrder): WorkOrder => ({ ...blank(), id: row.id, code: row.code, subject: row.subject, category: title(row.category), status: title(row.status), priority: title(row.priority), assignee: row.assigned_user_id || row.assigned_vendor_ref || '', assigneeType: row.assigned_user_id ? 'Person' : row.assigned_vendor_ref ? 'Vendor' : '', targetType: row.vehicle_id ? 'Vehicle' : 'Equipment', target: row.vehicle_id || row.equipment_id || '', openedAt: row.opened_at?.slice(0, 10) || '', dueAt: row.due_at?.slice(0, 10) || '', closedAt: row.completed_at?.slice(0, 10) || '', instructions: row.instructions || '', created: row.created_at, scheduleId: row.schedule_id })
...
export function MaintenanceWorkOrdersPage() {
  ...
  const save = async (event: FormEvent) => {
    ...
    if (editor.assigneeType === 'Person' && editor.assignee && !editor.assignee.startsWith('usr_')) { setNotice('Select a valid organization user. A person assignee must use a user ID beginning with usr_.'); return }
    if (editor.id) { setNotice('Use the work-order lifecycle actions to update an existing work order.'); return }
    try { const value = await maintenanceApi.createWorkOrder({ code: editor.code, subject: editor.subject, category: editor.category.toLowerCase().startsWith('preventive') ? 'preventive' : editor.category.toLowerCase(), priority: editor.priority.toLowerCase(), [editor.targetType.toLowerCase() === 'vehicle' ? 'vehicle_id' : 'equipment_id']: editor.target, currency: 'INR', estimated_cost_minor: 0, ...(editor.assigneeType === 'Person' && editor.assignee ? { assigned_user_id: editor.assignee } : {}), ...(editor.assigneeType === 'Vendor' && editor.assignee ? { assigned_vendor_ref: editor.assignee } : {}), ...(editor.dueAt ? { due_at: new Date(`${editor.dueAt}T23:59:59Z`).toISOString() } : {}), ...(editor.instructions ? { instructions: editor.instructions } : {}) }); await load(); setEditor(null); setNotice(`Work order ${value.code} created.`) }
